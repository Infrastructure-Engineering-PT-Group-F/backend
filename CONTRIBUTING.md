...
(`commitlint.config.js` + `.github/workflows/commitlint.yml`). Release automation
commits (`chore(release): release …`, generated by release-please) are exempt from
the issue-reference rule via the config's `ignores`.
